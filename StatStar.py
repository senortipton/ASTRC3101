...
"""
   ...
   This code was originally written in Fortran and then translated to C++ by:
   An Introduction to Modern Astrophysics
   Bradley W. Carroll and Dale A. Ostlie
   Addison Wesley
   Copyright 2007.

   It was then converted to Python 2 by Prof. D. Schiminovich at Columbia
   University. I have updated the code to work with Python 3 (data type 
   conversions and print() commands) and made it more user friendly 
   by adding a filename saving input for the user so they can repeatedly
   run the program without editing the filename in the code. Finally I
   edited the format of the data file so that the zone types convective
   and radiative are now labeled as "zone" and dlPdlT" now corresponds
   to the actual dlPdlT values.

   Notes from Prof. D. Schiminovich:
...
def StatStar(Msolar,Lsolar,Te,filename,X,Z):
...
#
      Ms = Msolar*Msun
      Ls = Lsolar*Lsun
      Rs = np.sqrt(Ls/(4*np.pi*cst.sigma))/(Te**2)
      Rsolar = Rs/Rsun
#
...
#      
      f.write('        r        Qm       L_r       T        P        rho      kap      eps     zone     dlPdlT\n')
...
#     Output to screen
      ...
      print ('      The model has been stored in '+filename+'.dat')
      print 
...
#
      ...
      if (getinp == 1):
           ...
           filename=input(' Enter the desired name for the data file without the filetype extension:')

      Igoof,ierr,istop=StatStar(Msolar,Lsolar,Te,filename,X,Z)
